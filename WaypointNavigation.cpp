...
namespace waypoint_navigation_lib{
    ...
    bool  WaypointNavigation::update(base::commands::Motion2D& mc){
        ...
        while ( distToNext <= corridor ){
            if( currentSegment < trajectory.size()-1){
                ...
                LOG_DEBUG_S << "Final phase:" << (finalPhase? "T" : "F") << std::endl <<
                    ...
                    "\t Pos. err. est.\t" << posErr << "/" << trajectory.back()->tol_position <<"m ";
                break;
            }
        }
        ...
        switch (currentState) {
            ...
            case ALIGNING:
                {
                    ...
                    if(pd_initialized){
                        ...
                        LOG_DEBUG_S << "d(heading err)/dt = " << headingErrDiff*180/M_PI << "deg/" << alignment_dt << "sec = " << headingErrDiff;
                    } else {
                        pd_initialized = true;
                        headingErrDiff = 0;
            ...
            case TARGET_REACHED:
                LOG_DEBUG_S << "Waypoint Navigation: Target Reached.";
                break;
            case NO_TRAJECTORY:
                LOG_DEBUG_S << "Invalid trajectory.";
                break;
            default:
                LOG_DEBUG_S << "Default case.";
                break;
        }
        return true;
    }
    ...
    void WaypointNavigation::initilalizeCurrentSegment(){
        ...
        for (size_t i = 1; i < trajectory.size(); i++) {
            ...
            LOG_DEBUG_S << "Segment: " << i << ", progress: " << progress <<
                ", dist from nominal " << distPerpend;
        }
